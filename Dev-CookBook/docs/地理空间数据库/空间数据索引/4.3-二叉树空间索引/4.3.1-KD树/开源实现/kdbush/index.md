...
`select()` 的通用算法基础是 [Floyd–Rivest 选择算法](/数据结构与算法/选择算法/Floyd-Rivest选择算法/)。通用算法负责在不完整排序整个数组的情况下确定第 `k` 小元素；KDBush 在此基础上针对二维点索引进行了适配：

- 通用算法比较单一数组中的 `A[i]`，KDBush 比较 `coords[2 * i + axis]`。
- `axis = 0` 时按 $x$ 坐标选择中位点，`axis = 1` 时按 $y$ 坐标选择中位点。
- 通用算法只交换数组元素，KDBush 必须同步交换 `ids[i]`、$x$ 坐标和 $y$ 坐标，保持原始 ID 与点坐标的对应关系。
- Floyd–Rivest 只负责一次第 `k` 小元素选择；KDBush 的 `sort()` 在左右区间上反复调用它，并交替切换分割轴，从而构建完整的扁平 KD 树。
- `nodeSize` 是 KDBush 控制 KD 递归何时停止的叶子块阈值，不属于 Floyd–Rivest 算法本身。

因此，Floyd–Rivest 是 KDBush 构建阶段使用的通用选择算法组件，而 KDBush 的二维数据布局、轴切换、递归建树和叶子块策略属于索引实现自身的设计。
